...
package com.beijing.beixin.utils.zxing.view;
...
public final class ViewfinderView extends View {
	 ...
	 * 扫描框中的中间线的宽度
	 */
	private static final int MIDDLE_LINE_WIDTH = 35;
	...
	public void onDraw(Canvas canvas) {
		...
		if (resultBitmap != null) {
			...
			if ((CaptureActivity.currentState != null) && (CaptureActivity.currentState.equals("onecode"))) {
				canvas.drawText(getResources().getString(R.string.scan_onecode), frame.left + 8,
						(float) (frame.bottom + (float) 40 * density), paint);

			}
			if ((CaptureActivity.currentState != null) && (CaptureActivity.currentState.equals("qrcode"))) {
				canvas.drawText(getResources().getString(R.string.scan_qrcode), frame.left - 20,
						(float) (frame.bottom + (float) 40 * density), paint);
			}
