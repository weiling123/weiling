...
public class DateToolUtil {
	...
	public static String GetDelayDate(String start, int delay) {
		...
		return calendar.get(Calendar.YEAR) + "-" + mm + "-" + dd + " 00:00:00";
	}
	...
	public static Date string2date(String time) {
		...
		if (temps.length > 1) {
			...
			time = temps[0] + " " + hour + ":" + min + ":00";
			// time = temps[0]+" "+"00:00:00";
	...
	public static boolean beyondTody(Date date) {
		...
		Date now = string2date(tyear + "-" + tmonth + "-" + tday + " 23:59:59");
	...
	public static int XiaoShiCha2(Date start, Date end) {
		long d = end.getTime() - start.getTime();
		if (d < 0) {
			d = 0;
		}
