...
public class ScrollViewContainer extends RelativeLayout {
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			} else if (((mBottomViewListener != null && mBottomViewListener.isCanSocroll() && mCurrentViewIndex == 1)
					|| canPullDown) && mCurrentViewIndex == 1 && mEvents == 0) {
				mMoveLen += (ev.getY() - mLastY);
				// 防止上下越界
				if (mMoveLen < -mViewHeight) {
					mMoveLen = -mViewHeight;
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (mPageChange != null) {
			mPageChange.onPageChange(mMoveLen == 0);
		}
	}
	...
	private BottomViewListener mBottomViewListener;
	...
	public void setBottomViewListener(BottomViewListener bottomViewListener) {
		mBottomViewListener = bottomViewListener;
	}
