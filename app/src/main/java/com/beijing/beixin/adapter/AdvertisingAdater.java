 ...
 * 
 */
public class AdvertisingAdater extends BaseAdapter {
	...
	public AdvertisingAdater(Context context, ListItemClickHelp callback) {
		super();
	...
	public View getView(final int position, View convertView, final ViewGroup arg2) {
		...
		bitmapUtil.displayImage(context, holder.iv_bookimage, list.get(position).getProductImageUrl());
		holder.tv_bookname.setText(list.get(position).getProductNm());
		holder.tv_bookprice.setText("¥" + Utils.parseDecimalDouble2(list.get(position).getPrice()));
		holder.tv_old_price.setText("¥" + Utils.parseDecimalDouble2(list.get(position).getOriginalUnitPrice()));
		...
		convertView.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				// list.get(position).getShopInfProxyList().size() != 0) {
				// bundle.putString("SkuId",
				// list.get(position).getShopInfProxyList().get(0).getSkuList().get(0)
				// .getSkuId());
				// }
